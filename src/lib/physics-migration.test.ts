...
const migrationPath = join(process.cwd(), "supabase/migrations/20260908000000_add_ib_physics_banks.sql");
const reconciliationPath = join(process.cwd(), "scripts/reconcile-supabase-assets.ts");
const productIds = ["bank_ib_physics_hl", "bank_ib_physics_sl", "bundle_ib_physics"];

describe("IB Physics billing migration", () => {
  it("adds Physics products and both bank constraints", () => {
    ...
    expect(migration).toContain("ib-physics-hl");
    expect(migration).toContain("ib-physics-sl");
